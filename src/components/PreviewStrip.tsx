...
// The benchmark's worth is a range from its own run, never one figure.
function BenchmarkValue({ runId }: { runId: number | null }) {
  const q = useQuery(() => (runId ? getBenchmarkSummary(runId) : Promise.resolve(null)), [runId])
  const s = q.data
  return <p data-testid="preview-value" data-tabular className="mt-2 text-sm">{s ? copy.benchmark.previewValue(formatMillions(s.savings_low), formatMillions(s.savings_high)) : copy.benchmark.cardNotRun}</p>
}
          ...
          <article key={row.case_key} data-testid={`preview-card-${row.case_key}`} className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-dashed border-border-strong bg-surface p-5">
            <div className="max-w-prose">
              <div className="flex flex-wrap items-center gap-2">
                <Pill tone="neutral">{copy.index.previewLabel}</Pill>
                {row.case_key === 'price_benchmark'
                  ? <><Pill tone="brand">{copy.benchmark.inDevelopment}</Pill><Pill tone="neutral">{copy.benchmark.lowConfidence}</Pill></>
                  : <Pill tone="brand">{copy.index.samplePill}</Pill>}
              </div>
              <h2 className="mt-3 text-lg font-semibold">{row.name}</h2>
              <p className="mt-1 text-sm text-text-muted">{row.case_key === 'price_benchmark' ? copy.benchmark.previewText : copy.index.previewText}</p>
              {row.case_key === 'price_benchmark'
                ? <BenchmarkValue runId={run?.id ?? null} />
