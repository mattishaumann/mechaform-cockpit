...
import { AGENTS, enabledAgents, previewAgents, previewPages } from '../lib/agents'

const group = 'px-3 pb-1 pt-5 font-mono text-xs uppercase tracking-widest text-text-muted first:pt-0'
    ...
    <div className="min-h-screen bg-bg text-text">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 md:flex-row md:gap-8 md:px-6 md:py-8">
        <nav aria-label="Main" className="-mx-4 flex shrink-0 gap-1 overflow-x-auto px-4 md:mx-0 md:w-52 md:flex-col md:overflow-visible md:px-0">
          ...
          {(previewAgents.length > 0 || previewPages.length > 0) && <p className={`hidden md:block ${group}`}>{copy.nav.preview}</p>}
          {previewAgents.map((k) => (
